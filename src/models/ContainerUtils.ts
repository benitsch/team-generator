export default class ContainerUtils {
  ...
  static groupElementsByProperty<KeyType>(
    array: Array<any>,
    propertyFn: (value: any) => KeyType,
  ): Map<KeyType, Array<any>> {
  ...
  static sortMapDescendingByKey(map: Map<number, any>): void {
    const descendingComparer = (
      pairA: [number, any],
      pairB: [number, any],
    ): number => {
      return pairB[0] - pairA[0];
    };
    const sortedMap: Map<number, any> = new Map<number, any>(
      [...map].sort(descendingComparer),
    );
    map = sortedMap;
  }
}
