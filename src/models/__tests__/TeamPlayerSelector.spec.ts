...
class OptimalTeamSelectorUnderTest extends OptimalTeamPlayerSelector {
  ...
  public validateSelectorInput(
    players: Player[],
    team: Team,
    minTeamSkill: number,
    maxTeamSkill: number,
  ): SelectorErrorCode | undefined {
    return super.validateSelectorInput(
      players,
      team,
      minTeamSkill,
      maxTeamSkill,
  ...
  public optimizePlayerSelectionOutOfRange(
    currentPlayerSelection: Player[],
    alternativePlayers: Player[],
    team: Team,
    minTeamSkill: number,
    maxTeamSkill: number,
  ): Player[] {
    return super.optimizePlayerSelectionOutOfRange(
...
describe('OptimalTeamPlayerSelectorInterfaceTest', () => {
  ...
  it('Shall not accept empty selectable player list', () => {
    ...
    const selectorResult: Array<Player> | SelectorErrorCode =
      selector.selectPlayers(
        new Array<Player>(),
        team,
        minTeamSkill,
        maxTeamSkill,
  ...
  it('Shall select exact missing amount of players within given skill range if possible.', () => {
    ...
    const selectResult: Array<Player> | SelectorErrorCode =
      selector.selectPlayers(
        selectablePlayers,
        team,
        minTeamSkill,
...
describe('OptimalTeamPlayerSelectorClassTest', () => {
  ...
  const selector: OptimalTeamSelectorUnderTest =
    new OptimalTeamSelectorUnderTest(instance(mockRandomSource));

  it('Shall not accept negative team skill range', () => {
    const minTeamSkill: number = -10;
    const maxTeamSkill: number = 10;

    const validateResult: SelectorErrorCode | undefined =
      selector.validateSelectorInput(
        selectablePlayers,
        team,
        minTeamSkill,
        maxTeamSkill,
  ...
  it('Shall not accept selecting players if team is already full!', () => {
    ...
    for (let i = fullTeam.allPlayers.length; i < fullTeam.targetSize; i++) {
      //fill up team
      const player: Player = new Player('Player' + i);
  ...
  it('Shall not accept that selectable player list contains team members.', () => {
    ...
    const selectablePlayerListWithTeamMembers: Array<Player> =
      new Array<Player>();
    ...
    const validateResult: SelectorErrorCode | undefined =
      selector.validateSelectorInput(
        selectablePlayerListWithTeamMembers,
        team,
        minTeamSkill,
        maxTeamSkill,
      );
  ...
  it('Shall ensure that random player selection part is based on a random source.', () => {
    const amountOfPlayersToSelect: number = 3;
    ...
    const [randomPlayerSelection, alternativePlayers] =
      selector.selectRandomPlayers(selectablePlayers, amountOfPlayersToSelect);
  ...
  it('Shall not optimize player selection if current selection within given range.', () => {
    ...
    const optimizedSelection: Array<Player> =
      selector.optimizePlayerSelectionOutOfRange(
        currentPlayerSelection,
        alternativePlayerSelection,
        teamToSelectPlayersFor,
        minTeamSkill,
        maxTeamSkill,
      );
    expect(optimizedSelection.length).toEqual(
      teamToSelectPlayersFor.targetSize - teamToSelectPlayersFor.currentSize,
    );
