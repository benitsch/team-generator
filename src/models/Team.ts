...
export default class Team {
  ...
  addPlayer(player: Player): boolean {
    return !this.isFull &&
      !this.isPlayerInTeam(player) &&
      player.isSkillAssessedForGame(this.game)
      ? (this._playerList.push(player), true)
      : false;
  }
  ...
  addSubstitutionPlayer(player: Player): boolean {
    return !this.isFull &&
      !this.isPlayerInTeam(player) &&
      player.isSkillAssessedForGame(this.game)
      ? (this._substitutionPlayerList.push(player), true)
      : false;
  }
  ...
  removePlayer(player: Player): void {
    ...
    const listIndexSubPlayer: number = this._substitutionPlayerList.indexOf(
      player,
      0,
    );
  ...
  isPlayerInTeam(player: Player): boolean {
    return (
      this._playerList.includes(player) ||
      this._substitutionPlayerList.includes(player)
    );
  }
