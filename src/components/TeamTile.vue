                        ...
                        <template #title>
                          <v-row no-gutters align="center">
                            ...
                            <div>
                              ...
                              ><span>{{ getSkillByPlayerId(item.value) }}</span>
                            </div>
                          </v-row>
                        </template>
                      </v-list-item>
...
<script setup lang="ts">
  ...
  function isItemChecked(playerId: string): boolean {
    return (
      selectedNewPlayers.value.find((item) => playerId === item) !== undefined
    );
  }

  function getSkillByPlayerId(playerId: string): number | undefined {
    const player = getAllPossiblePlayersToAdd().find(
   ...
   * */
  function getAllPossiblePlayersToAdd(): Player[] {
    return state.players.filter((player) => {
      return (
        !team.isPlayerInTeam(player as any as Player) &&
        !oppositeTeam?.isPlayerInTeam(player as any as Player)
      );
    }) as any as Player[];
  }

  function manuallyAddPlayerToTeam(): void {
    selectedNewPlayers.value.forEach((playerId) => {
      const player = getAllPossiblePlayersToAdd().find(
        (p) => p.id === playerId,
      );
      if (addSubplayer.value) {
        team.addSubstitutionPlayer(player as any as Player);
      } else {
        team.addPlayer(player as any as Player);
      }
    });
  ...
  function getPlayerTitleAttr(player: Player): string {
